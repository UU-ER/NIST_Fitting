...
flag_extra_gas = 0; 
count_material_CH4_combinations = 0;
...
%%
for every_gas = [1:length(gas_display)] %%%%%%%%%%%% I changed this since I only wanted the CH4
    for material_number = [1:row_length_table_NIST_1] %for every material (every row in gas)(26)
        start_j = 1;
        try
            place_T_in_structure = data_NIST.(material_name(material_number)).(gas_display(every_gas));
            if gas_display(every_gas) == "Methane"
                count_material_CH4_combinations = count_material_CH4_combinations+1;
            end
            ...
            for temperature = [1: length(unique_temperatures_1_gas)]
                ...
                if gas_display(every_gas) == "Methane"
                    flag_CH4 = 1;
                    doi_list_CH4_one_mat = [doi_list_CH4_one_mat doi]; % save all the dois in one vector special for CH4
                end
            end
            ...
            if flag_CH4 ==1
                unique_doi_list_CH4_one_mat = unique(doi_list_CH4_one_mat);
            end
            
            doi_list = [doi_list, unique_doi_list_one_mat];
            if flag_CH4 ==1
                doi_list_CH4 = [doi_list_CH4, unique_doi_list_CH4_one_mat];
            end
...
for every_gas = [1:length(gas_display)] %--> again onlye CH4
    total_isotherms_comp_gas = total_isotherms_comp_gas + number_of_isotherms_per_gas(every_gas);
    if gas_display(every_gas) == "Methane"
        isotherms_CH4 = number_of_isotherms_per_gas(every_gas);
    end
end

doi_list_unique = unique(doi_list) ; %number of unique dois
doi_list_CH4_unique = unique(doi_list_CH4); % number of unique dois for CH4 
...
fprintf("Total number of isotherms for CH4  %d  \n", isotherms_CH4)
fprintf("Total number of materials for CH4  in new structure %d  \n", count_material_CH4_combinations)
fprintf("Total number of different DOIs for CH4  %d  \n", length(doi_list_CH4_unique) )
fprintf("Total number of DOIs for CH4 (counted multiple times if DOI for multiple materials) %d  \n", length(doi_list_CH4) )
end