        ...
        if isempty(list_unit_amount_adsorbed)==0 
            ...
            if all(ismember(list_unit_amount_adsorbed, list_convertible_units))== 1 
                ...
                if isfield(fitting_input,(every_gas_name)) == 1 
                   ...
                   try 
                       [xm_t,ym_t,ymSpec_t,sigmaCH4_t,p_t,R2] = main_fitting_Nist_5(fitting_input,temperature_name_for_input,every_gas_name,'toth');%''
                       R2_toth =R2;
                       model = 'toth';
                       ...
                       [xm_t2,ym_t2,ymSpec_t2,sigmaCH4_t2,p_t2,R22] = main_fitting_Nist_5(fitting_input,temperature_name_for_input,every_gas_name,'langm_freund');%''
                       R2_langfr = R22;
                       ...
                       if error_R2_toth >= 0.02 
                           [xm_s,ym_s,ymSpec_s,sigmaCH4_s,p_s,R2] = main_fitting_Nist_5(fitting_input,temperature_name_for_input,every_gas_name,'s_shaped');
                           R2_s_shaped = R2;
                           error_R2_s_shaped = 1 - R2_s_shaped;
                           if error_R2_toth >= error_R2_s_shaped % take s_shape
                               model = 's_shaped';
                               flag_s_shape = 1;
                               R2 = R2_s_shaped;sigmaCH4 = sigmaCH4_s; p = p_s;
                               ...
                               if R2_toth > R2_langfr
                                   ...
                                   R2 = R2_toth; sigmaCH4 = sigmaCH4_t; p = p_t;
                                ...
                                list_all_units_amount_adsorbed = [list_all_units_amount_adsorbed list_unit_amount_adsorbed];
                                 if plot_isotherms == "yes"
                                     ...
                                     for every_temp = [1:length(temperature_name_for_input)]
                                          ...
                                          xm = [zero_list_to_append; xm] ; ym = [zero_list_to_append ;ym ] ; sigmaCH4 = [zero_list_to_append; sigmaCH4];  
                                          specific_ymSpec = [0;ymSpec{:,every_temp}];
                                          plot(xm(1:length(specific_ymSpec),every_temp),sigmaCH4(1:length(specific_ymSpec),every_temp),'color', colour(every_temp), 'LineStyle', '-');
                                    ...
                                    if plot_isotherms == "yes" 
                                        ...
                                        colour = ['b', 'r','m','y', 'k', 'c'];
                                         for every_temp = [1:length(temperature_name_for_input)]
                                              ...
                                              xm = [zero_list_to_append; xm] ; ym = [zero_list_to_append ;ym ] ; sigmaCH4 = [zero_list_to_append; sigmaCH4]; 
                                              plot(xm(1:length(ymSpec{:,every_temp}),every_temp),sigmaCH4(1:length(ymSpec{:,every_temp}),every_temp),'color', colour(every_temp), 'LineStyle', '-');
                                              temperature_to_display_pre = string(temperature_name_for_input(every_temp));
                                              temperature_name_to_display = strip(temperature_to_display_pre, 'left', 'T');
                                              temperature_name_to_display = strip(temperature_name_to_display, 'left', 'a');
                                              temperature_name_to_display = strip(temperature_name_to_display, 'left', 'A');
