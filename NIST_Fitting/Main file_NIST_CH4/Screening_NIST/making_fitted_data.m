...
if flag_fitted == 1 && flag_not_fitted == 0
  ...
  [KH_save, ns_equ_save, q0_save, m0_save, gamma_save, wc_save,dH_save,T0] = characteristics_4(p,model,pCH4_1,rhoMat,[],120) ;
  if wc_save < 10e14
    flag_wc = 1;
    structure_exists = 0;
    ...
    for every_temp = [1:length(temperature_name_for_input)]
         values_to_put_in = double(fitting_input.Methane.(temperature_name_for_input(every_temp))); 
