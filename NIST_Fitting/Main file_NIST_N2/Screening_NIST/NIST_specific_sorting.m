...
%  %NUM 8
%  structure_from_NIST.Methane.Carbon.ozdvzow___b__k__c__s_dwzofdhvdwdhss  = rmfield(structure_from_NIST.Methane.Carbon.ozdvzow___b__k__c__s_dwzofdhvdwdhss  , "TaAa196" ) ;
%  structure_from_NIST.Methane.Carbon.ozdvzow___b__k__c__s_dwzofdhvdwdhss   = rmfield(structure_from_NIST.Carbon_Dioxide.Carbon.ozdvzow___b__k__c__s_dwzofdhvdwdhss  , "TaAa196num6457" );
...
count_dois = 0;
material_list = fieldnames(structure_from_NIST.Nitrogen);
...
for every_material =  [1:length(material_list)]
    ...
    doi_list = fieldnames(structure_from_NIST.Nitrogen.(material_name));
    for every_doi = [1:length(doi_list)]
        newtest_count_debug = newtest_count_debug + 1;
        doi_name = string(doi_list(every_doi));
        try
            char_list = fieldnames(char_struct_from_NIST.Nitrogen.(material_name).(doi_name));
            flag_char_exist = 1;
            count_strings = 0;
            for every_char = [1:length(char_list)]
                characteristic = string(char_list(every_char));
                value_characteristic = char_struct_from_NIST.Nitrogen.(material_name).(doi_name).(characteristic);
            ...
            if flag_char_exist == 1 % if all characteristics are numbers
                ...
                temperature_list = fieldnames(structure_from_NIST.Nitrogen.(material_name).(doi_name));
                for every_temp = [1:length(temperature_list)]
                    temperature_name = string(temperature_list(every_temp));
                    structure_from_NIST_new.(material_name).Nitrogen.(temperature_name) = structure_from_NIST.Nitrogen.(material_name).(doi_name).(temperature_name);
                    structure_from_NIST_new.(material_name).Nitrogen.(temperature_name).Doi_NIST = doi_name;
                end
            else
