...
flag_extra_gas = 0; 
count_material_N2_combinations = 0;
...
%%
for every_gas = [1:length(gas_display)] %%%%%%%%%%%% I changed this since I only wanted the N2
    for material_number = [1:row_length_table_NIST_1] %for every material (every row in gas)(26)
        start_j = 1;
        try
            place_T_in_structure = data_NIST.(material_name(material_number)).(gas_display(every_gas));
            if gas_display(every_gas) == "Nitrogen"
                count_material_N2_combinations = count_material_N2_combinations+1;
            end
            ...
            for temperature = [1: length(unique_temperatures_1_gas)]
                ...
                if gas_display(every_gas) == "Nitrogen"
                    flag_N2 = 1;
                    doi_list_N2_one_mat = [doi_list_N2_one_mat doi]; % save all the dois in one vector special for N2
                end
            end
            ...
            if flag_N2 ==1
                unique_doi_list_N2_one_mat = unique(doi_list_N2_one_mat);
            end
            
            doi_list = [doi_list, unique_doi_list_one_mat];
            if flag_N2 ==1
                doi_list_N2 = [doi_list_N2, unique_doi_list_N2_one_mat];
            end
...
for every_gas = [1:length(gas_display)] %--> again onlye N2
    total_isotherms_comp_gas = total_isotherms_comp_gas + number_of_isotherms_per_gas(every_gas);
    if gas_display(every_gas) == "Nitrogen"
        isotherms_N2 = number_of_isotherms_per_gas(every_gas);
    end
end

doi_list_unique = unique(doi_list) ; %number of unique dois
doi_list_N2_unique = unique(doi_list_N2); % number of unique dois for N2 
...
fprintf("Total number of isotherms for N2  %d  \n", isotherms_N2)
fprintf("Total number of materials for N2  in new structure %d  \n", count_material_N2_combinations)
fprintf("Total number of different DOIs for N2  %d  \n", length(doi_list_N2_unique) )
fprintf("Total number of DOIs for N2 (counted multiple times if DOI for multiple materials) %d  \n", length(doi_list_N2) )
end